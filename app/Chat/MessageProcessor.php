...
class MessageProcessor {
    ...
    public function extractEntities(string $message): array {
        ...
        // استخراج التواريخ
        $datePattern = '/(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}|\d{1,2}\s+(يناير|فبراير|مارس|أبريل|مايو|يونيو|يوليو|أغسطس|سبتمبر|أكتوبر|نوفمبر|ديسمبر|January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{2,4})/i';
        preg_match($datePattern, $message, $dateMatches);
        if (!empty($dateMatches)) {
            $entities['date'] = $dateMatches[0];
        }
        
        // استخراج الأرقام (كميات، أسعار)
        $numberPattern = '/\d+(?:\.\d+)?/';
