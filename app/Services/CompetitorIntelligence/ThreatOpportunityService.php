...
class ThreatOpportunityService {
    ...
    public function scanCompetitor(Competitor $competitor, int $days = 30): array {
        ...
        if ($createdAt && strtotime($createdAt) >= strtotime('-14 days') && count($changes) >= 1) {
            $insights[] = $this->save($userId, $websiteId, $competitorId, 'threat', "New market entrant: {$name}",
                ...
                'Competitor added_at=' . $createdAt . "; changes_in_period={$this->evidenceList(array_slice($changes, 0, 5))}", 'medium', 'medium',
        ...
        if (count($criticalOrHigh) >= 3) {
            $insights[] = $this->save($userId, $websiteId, $competitorId, 'threat', "Aggressive activity from {$name}",
                count($criticalOrHigh) . " high/critical-severity changes detected in the last {$days} days.",
                $this->evidenceList($criticalOrHigh), 'high', 'high',
        ...
        if (!empty($pricingDrops)) {
            $insights[] = $this->save($userId, $websiteId, $competitorId, 'threat', "Pricing change from {$name}",
                'Competitor changed publicly listed pricing.', $this->evidenceList($pricingDrops), 'medium', 'medium',
        ...
        if (count($newOffers) >= 2) {
            $insights[] = $this->save($userId, $websiteId, $competitorId, 'threat', "Repeated new offers from {$name}",
                count($newOffers) . ' offer changes detected - competitor is actively promoting.',
                $this->evidenceList($newOffers), 'medium', 'medium',
        ...
        if (!empty($removedPages)) {
            $insights[] = $this->save($userId, $websiteId, $competitorId, 'opportunity', "{$name} removed a service/page",
                'A previously monitored page for this competitor is no longer reachable - may indicate a discontinued service.',
                $this->evidenceList($removedPages), 'low', null,
        ...
        if ($noActivity && $everMonitored) {
            $insights[] = $this->save($userId, $websiteId, $competitorId, 'opportunity', "Weak recent activity from {$name}",
                "No changes detected across monitored public pages in the last {$days} days.",
                'Based on ' . $days . ' days of monitoring with no recorded ci_changes rows.', 'low', null,
