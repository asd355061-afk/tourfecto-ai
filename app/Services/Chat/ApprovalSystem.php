    ...
    public function approve(int $messageId, int $userId): array {
        try {
            ...
            if (!empty($msg['ai_reply_generated'])) {
                // تصحيح مهم: ChatManager::sendMessage() العادية بتستخدم
                // instance عام واحد فاضي من UltraMsg (مفيش instance_id/api_key
                // افتراضيين)، يعني كانت بتفشل بصمت لأي عميل حقيقي (كل عميل
                // له حساب UltraMsg خاص بيه مربوط بـ website_id بتاعه في
                // platform_connections). لازم نستخدم sendMessageForWebsite()
                // اللي بتجيب بيانات الاعتماد الصح لنفس الموقع.
                $chatManager = new ChatManager();
                if (($msg['platform'] ?? '') === 'ultramsg' || ($msg['platform'] ?? '') === 'whatsapp') {
                    $sent = $chatManager->sendMessageForWebsite(
                        (int) $msg['website_id'],
                        $msg['customer_phone'],
                ...
                if ($sent) {
                    ...
                    Logger::warning('Approved reply failed to send', ['message_id' => $messageId, 'website_id' => $msg['website_id'] ?? null]);
                }
            }
            ...
            return [
                ...
                'status' => 'approved',
                'sent' => $sent ?? false,
                'message' => ($sent ?? false) ? 'تمت الموافقة والإرسال بنجاح.' : 'تمت الموافقة لكن تعذر إرسال الرسالة فعليًا - تأكد إن واتساب لسه متصل.'
            ];
...
class NotificationService {
    ...
    public function sendApprovalNotification(int $userId, int $messageId): void {
        try {
            // تصحيح: كان بس بيسجّل في app.log ومفيش أي إشعار حقيقي بيوصل
            // للعميل - يعني جرس الإشعارات في الداشبورد فاضي دايمًا حتى لو
            // فيه رسايل واتساب مستنية موافقته.
            if (class_exists('Notification')) {
                Notification::notify(
                    $userId,
                    'chat_pending_approval',
                    'رسالة واتساب جديدة تحتاج موافقتك',
                    'عميل بعتلك رسالة والذكاء الاصطناعي جهّز رد مقترح - راجعه وأرسله.',
                    '/chat/pending'
                );
            }
