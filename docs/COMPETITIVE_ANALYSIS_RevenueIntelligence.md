...
| G2 | **الإيراد حسب المنتج/الخدمة** | كلهم | صراحة غير متاح: `getRevenueByProduct()` ترجع "Not enough data" لعدم وجود كتالوج منتجات مرتبط بالإيراد في المنصة. |
...
| G6 | **اتساع المعايير** (Benchmarks) | كلهم (معايير غنية عبر مقاييس/صناعات) | الجدول يدعم مقاييس متعددة، لكن سكربت إعادة البناء ينتج حاليًا مقياسًا واحدًا فقط (growth_percent_monthly) (cron/revai_benchmarks_rebuild.php). |
| G7 | **أهداف/حصص المبيعات (Quotas/Goals)** | كلهم | لا يوجد تتبع حصص/أهداف للمندوبين أو الفرق في الموديول (بينما المنصة لديها CrmSalesGoal لكن الموديول لا يقرأه). |
